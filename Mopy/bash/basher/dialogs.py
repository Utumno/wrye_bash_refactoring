...
class ImportFaceDialog(DialogWindow):
    ...
    def DoImport(self):
        ...
        bosh.faces.PCFaces.save_setFace(self.fileInfo, self.fdata[item],
                                        pc_flags)
        balt.showOk(self, _(u'Face imported.'), self.fileInfo.ci_name)
        self.accept_modal()
...
class CreateNewPlugin(DialogWindow):
    ...
    def _handle_ok(self):
        ...
        chosen_name = ModInfo.new_name(self._plugin_name.text_content +
            self._plugin_ext.get_value(), pw.data_store)
